...
import { getListing ,getImages ,deleteListingImage ,updateListing} from '../APIs'
...
const UpdateProperty = () => {
    ...
    const dropzoneChildren = (status: DropzoneStatus, theme: MantineTheme) => (
        <Group position="center" spacing="xl" style={{ minHeight: 220, pointerEvents: 'none' }}>
          ...
          <div className='text-center'>
              ...
              : 
              <Text size="xl" inline>
              Click here to update thumbnail image
            </Text>
    ...
    const [property_images, setImage] = useState([]);
    const [property,setProperty] = useState({})
    const {property_id} = useParams()
    const id:any = property_id
    ...
    const deleteImage = (id:number) => {
        ...
        deleteListingImage('test tpken',id)
        .then((data)=>{
            if(data.error){
                setError(data.error)
            }
            ...
            alert('Kindly ensure there is atleast one image available after deletion of this item')
        }
    }
    ...
    const retrieveListing = (id:number) => {
        ...
        .then((data:any)=>{
            if(data.error){
                ...
                setValues({...values,
                    ...
                    current_thumbnail:data.thumbnail,
                    ...
                    has_pool:data.has_pool === true ? 1 : 0,
                    has_parking:data.has_parking === true ? 1 : 0,
                    has_gym:data.has_gym === true ? 1 : 0,
                    is_furnished:data.is_furnished === true ? 1 : 0,
                    
                    price:data.price,
                    
                    off_plan:data.off_plan || 0,formData: new FormData()
    ...
    const submitRequest = () => {
        ...
        formData.set('listing_title',listing_title)
        for (var x = 0; x < images.length; x++) {
            formData.append('images',images[x]);
            
        }
        updateListing('test token',formData)
            .then((data:any) => {
                if (data.error) {
                    setError(data.error)
...
return(
   ...
   className="w-100"
    ...
    const existingPropertyImages = () => {
        return(
            <div className='row p-4'>
              
                {property_images.map((image:any,i:number)=> (
                ...
                <div className="col-6 py-3" >
                   ...
                   <div className="row">
                       ...
                       <i className="icofont-close-circled" onClick={() => {if(window.confirm('Are you sure you want to delete this image?')){deleteImage(image.image_id)};}} style={{color:'red',fontWeight:'bold',cursor:'pointer'}}></i>
                       </div>
                       <div className="col-12">
                       <img className="img-fluid w-100" src={image.url} style={{height:'150px',border:'2px solid black'}}/>
                       </div>
    ...
    return(
        ...
        <AdminLayout>
          ...
          />
            ...
            <Tabs.Tab label="Update Property Images" icon={<Edit size={14} />}>
